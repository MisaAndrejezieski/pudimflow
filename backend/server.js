// backend/server.js
// PudimFlow API - Versão definitiva
...
// SUA conexão direta do Neon (sem SSL problemático)
  ...
  try {
    ...
    if (result.rows.length > 0) {
      ...
      res.json({ success: true, data: {
        brascopEstantes: row.brascop_estantes, brascopKg: row.brascop_kg,
        brascopParadas: row.brascop_paradas, brascopPerdas: row.brascop_perdas,
        seracEstantes: row.serac_estantes, seracKg: row.serac_kg,
        seracParadas: row.serac_paradas, seracPerdas: row.serac_perdas,
        totalEstantes: row.total_estantes, totalKg: row.total_kg, totalPerdas: row.total_perdas,
        bpfOk: row.bpf_ok, bpfObservacao: row.bpf_observacao, falhas: row.falhas, reclamacoes: row.reclamacoes,
        dssRealizado: row.dss_realizado, dssTema: row.dss_tema, quaseAcidente: row.quase_acidente,
        etiquetaAzul: row.etiqueta_azul, etiquetaVermelha: row.etiqueta_vermelha,
        etiquetaFechada: row.etiqueta_fechada, verAgirAntes: row.ver_agir_antes, verAgirDepois: row.ver_agir_depois
      }});
    } else {
